...
import warnings
warnings.filterwarnings('ignore', category=UserWarning, message='.*Tried to instantiate.*')

# Configuration
class Config:
    MODEL = "llma3.2:latest"
    EMBEDDING_MODEL_NAME = "sentence-transformers/all-MiniLM-L6-v2"
    OLLAMA_API_BASE_URL = "http://localhost:11434"
    HUGGING_FACE_EMBEDDINGS_DEVICE_TYPE = "cuda" if torch.cuda.is_available() else "cpu"
...
def load_embedding_model(model_name, normalize_embedding=True):
    ...
    hugging_face_embeddings = HuggingFaceEmbeddings(
        ...
        encode_kwargs={
            ...
            'batch_size': 32  # Ajuste selon ta RAM GPU
        }
    )
...
def check_cuda():
    ...
    - Version CUDA: {torch.version.cuda if torch.cuda.is_available() else 'N/A'}
